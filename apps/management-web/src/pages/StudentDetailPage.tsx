...
export function StudentDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [profile, enrollmentPage, coursePage] = await Promise.all([
      ...
      const lifecycle = await Promise.all(
        enrollmentPage.content.flatMap(enrollment => [
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ArrowLeft size={18} /> Quay lại danh sách học viên
        </Link>
      </div>

      {/* Main Category Tabs Navigation */}
      <nav className="flex border-b border-outline-variant/40 overflow-x-auto no-scrollbar gap-3 mb-6 bg-surface p-1.5 rounded-2xl border">
        <button
        ...
        >
          <CalendarBlank size={18} /> Lịch sử học vụ & Khóa học
          {enrollments.length > 0 && (
            <span
              className={`ml-1 px-2 py-0.5 text-xs rounded-full font-black ${
                activeTab === "history" ? "bg-on-primary/20 text-on-primary" : "bg-primary-container/30 text-primary"
              }`}
            >
              {enrollments.length}
            </span>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-primary-container/20 px-3 py-1 text-xs font-bold text-primary font-mono">
                      {student.studentCode}
                    </span>
                    ...
                    <span className="rounded-full bg-surface-container px-3 py-1 text-xs font-bold text-on-surface-variant">
                      Gia nhập: {date(student.joinedAt)}
                    </span>
                  </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
            <div className="space-y-6">
              <Panel
                ...
                subtitle="Danh sách các khóa học học viên đã và đang tham gia trong quá trình học tập tại trung tâm."
              >
                <div className="space-y-4">
                  {enrollments.map(enrollment => {
                    ...
                    return (
                      <article key={enrollment.id} className="rounded-2xl border border-outline-variant/30 bg-surface p-5 transition hover:border-primary/30">
                        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                          <div>
                            ...
                            <h3 className="font-display text-lg font-bold">{course?.name ?? "Khóa học"}</h3>
                            <p className="text-sm text-on-surface-variant">
                              Mã lớp: {course?.code} · Ngày bắt đầu: {date(enrollment.startedOn)}
                            </p>
                            {enrollment.notes && <p className="mt-2 text-sm bg-surface-container-low/50 p-2.5 rounded-xl border border-outline-variant/20">{enrollment.notes}</p>}
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {["ACTIVE", "PAUSED"].includes(enrollment.status) && (
                              <>
                                <button
                                  onClick={() => setDialog({ kind: "reservation", enrollment })}
                                  className="inline-flex items-center gap-2 rounded-full border border-outline-variant/60 px-4 py-2 text-sm font-semibold hover:bg-surface-container transition"
                                >
                                  <PauseCircle size={18} /> Bảo lưu
                                </button>
                                <button
                                  onClick={() => setDialog({ kind: "transfer", enrollment })}
                                  className="inline-flex items-center gap-2 rounded-full border border-outline-variant/60 px-4 py-2 text-sm font-semibold hover:bg-surface-container transition"
                                >
                                  <ArrowsLeftRight size={18} /> Chuyển lớp
                                </button>
                              </>
                            )}
                          </div>
                        ...
                        {canApprove && (reservation || transfer) && (
                          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-surface-container-low p-3.5 text-sm border border-outline-variant/30">
                            <span className="font-medium">{reservation ? `Yêu cầu bảo lưu: ${reservation.reason}` : `Yêu cầu chuyển lớp: ${transfer?.reason}`}</span>
                            <div className="flex gap-2">
                              <button
                                onClick={() => void reject(reservation, transfer, load, setError)}
                                className="inline-flex items-center gap-1.5 rounded-full border border-error/40 px-4 py-2 font-semibold text-error hover:bg-error-container/20 transition"
                              >
                                <XCircle size={18} /> Từ chối
                              </button>
                              <button
                                onClick={() => void approve(reservation, transfer, load, setError)}
                                className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 font-semibold text-on-primary hover:opacity-90 transition"
                              >
            ...
            <aside className="space-y-6">
              ...
              <Panel title="Quy trình duyệt học vụ">
                <p className="text-sm leading-6 text-on-surface-variant">
                  Mỗi đợt bảo lưu hoặc chuyển lớp phải gửi qua hệ thống yêu cầu và chờ cấp quản lý phê duyệt. Không tự ý chỉnh sửa lớp mà không lưu vết.
                </p>
              </Panel>
            </aside>
          </div>
...
async function reject(
  ...
  try {
    if (reservation) await apiFetch(`/admin/enrollments/reservations/${reservation.id}/reject`, { method: "PATCH" });
    else if (transfer) await apiFetch(`/admin/enrollments/transfers/${transfer.id}/reject`, { method: "PATCH" });
    await reload();
  } catch (value) {
    setError(value instanceof Error ? value.message : "Không thể từ chối yêu cầu");
  }
}
...
function LifecycleDrawer({
  ...
  const targets = useMemo(
    () => courses.filter(item => item.id !== dialog.enrollment.courseId && ["OPEN", "ACTIVE"].includes(item.status)),
    [courses, dialog.enrollment.courseId]
  );
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await apiFetch(`/admin/enrollments/${dialog.enrollment.id}/${transfer ? "transfers" : "reservations"}`, {
        ...
        body: JSON.stringify(
          transfer
            ? { targetCourseId, reason, feeAdjustment: 0, notes: notes || null }
  ...
  return (
    <Drawer
      title={dialog.kind === "reservation" ? "Tạo yêu cầu bảo lưu" : "Tạo yêu cầu chuyển lớp"}
      description="Yêu cầu được lưu vào lịch sử và chỉ có hiệu lực sau khi quản lý duyệt."
      onClose={onClose}
    >
      <form onSubmit={submit} className="flex h-full flex-col gap-5">
          ...
          {dialog.kind === "transfer" && (
            <Field label="Lớp đích">
              <select
                ...
                className="input border border-outline-variant/60 rounded-xl px-3 py-2 w-full"
              >
        ...
        <div className="flex justify-end gap-3 border-t border-outline-variant/30 pt-4">
          <button type="button" onClick={onClose} className="rounded-full border px-5 py-2.5 font-semibold">
            Hủy
          </button>
          <button disabled={saving} className="rounded-full bg-primary px-5 py-2.5 font-semibold text-on-primary disabled:opacity-50">
...
function Panel({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-outline-variant/30 bg-surface p-5 shadow-sm md:p-6">
      <h2 className="font-display text-xl font-bold">{title}</h2>
      {subtitle && <p className="mb-5 mt-1 text-sm text-on-surface-variant">{subtitle}</p>}
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-surface-container-low p-4 border border-outline-variant/20">
      ...
      <strong className="mt-2 block text-2xl text-on-surface">{value}</strong>
    </div>
  );
}
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-b border-outline-variant/20 py-3 last:border-0">
      <span className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant">{label}</span>
      <span className="mt-1 block text-sm font-semibold text-on-surface">{value}</span>
    </div>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-on-surface-variant">{label}</span>
      {children}
    </label>
  );
}
function State({ text, error }: { text: string; error?: boolean }) {
  return (
    <div className={`rounded-2xl border p-10 text-center ${error ? "border-error/30 bg-error-container/10 text-error" : "border-outline-variant/30 text-on-surface-variant"}`}>
      {text}
    </div>
  );
}
