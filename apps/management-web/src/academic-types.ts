...
export type Course = {
  id: string; code: string; name: string; description: string | null; level: string | null;
  targetBand: number | null; totalSessions: number | null; tuitionAmount: number | null;
  isPublic: boolean; isActive: boolean; createdAt: string; updatedAt: string;
};

export type CourseForm = {
  code: string; name: string; description: string; level: string; targetBand: string;
  totalSessions: string; tuitionAmount: string; isPublic: boolean; isActive: boolean;
};

export type ClassStatus = "PLANNED" | "ENROLLING" | "IN_PROGRESS" | "COMPLETED" | "CANCELLED";
export type AcademicClass = {
  id: string; courseId: string; code: string; name: string; capacity: number; startsOn: string;
  endsOn: string | null; status: ClassStatus; defaultZoomUrl: string | null; createdAt: string; updatedAt: string;
};

export type ClassForm = {
  courseId: string; code: string; name: string; capacity: string; startsOn: string;
  endsOn: string; status: ClassStatus; defaultZoomUrl: string;
};

export type EnrollmentStatus = "PENDING" | "ACTIVE" | "COMPLETED" | "CANCELLED" | "DROPPED";
export type Enrollment = {
  id: string; classId: string; studentId: string; status: EnrollmentStatus;
...
export const enrollmentStatusLabel: Record<EnrollmentStatus, string> = { PENDING: "Chờ xác nhận", ACTIVE: "Đang học", COMPLETED: "Hoàn thành", CANCELLED: "Đã hủy", DROPPED: "Đã nghỉ" };
