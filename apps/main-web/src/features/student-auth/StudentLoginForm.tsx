...
import { ArrowLeft, ArrowRight, CircleNotch, LockKey } from "@phosphor-icons/react";
...
export function StudentLoginForm() {
  const { completeOnboarding, isConfigured, isLoading, session, signIn, signInWithOAuth } = useStudentSession();
  ...
  return <StudentAuthShell compact>
    ...
    {registered ? <StudentFormNotice compact kind="success">Tài khoản đã được tạo. Hãy kiểm tra email để xác nhận, sau đó đăng nhập.</StudentFormNotice> : null}
    ...
    <form className="space-y-3" onSubmit={submit} noValidate>
      <div>
        ...
        <input id="student-email" type="email" autoComplete="email" inputMode="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="ban@example.com" required disabled={!isConfigured || submitting} aria-invalid={Boolean(fieldErrors.email)} aria-describedby={fieldErrors.email ? "student-email-error" : undefined} className="min-h-11 w-full rounded-xl border border-[var(--border)] bg-white px-4 text-base text-[var(--text)] outline-none transition placeholder:text-[#aaa2a5] focus:border-[var(--brand-pink)] focus:ring-2 focus:ring-[var(--brand-pink-soft)] disabled:cursor-not-allowed disabled:bg-[var(--surface-muted)]" />
